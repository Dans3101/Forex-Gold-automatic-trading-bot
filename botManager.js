...
let firstRun = true; // 🔑 local flag (no need to set in Render)
...
async function runScraper(bot) {
  ...
  try {
    ...
    if (signals.length === 0) {
      ...
      if (!firstRun) await sendTelegramMessage(bot, "ℹ️ No chat signals this cycle.");
    } else {
    ...
    firstRun = false; // 🔑 after the first cycle, this stays false
  }
}
